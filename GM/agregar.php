...
$sqlCedula = "SELECT * FROM usuario WHERE numero_identidad = '$numeroIdentidad' AND tipo_documento = '$tipoDocumento'";
...
$idUsuario = $rowCedula['id_usuario']; // Asumiendo que el campo de la tabla de usuario es 'id_usuario'
...
if ($tipoSeguro == 'Transporte') {
    ...
    if ($conn->query($sqlInsertVehiculo) === TRUE) {
        ...
        $sqlInsertSeguroVehiculo = "INSERT INTO segurovehiculo (Id_Aseguradora, Id_Vehiculo, Id_Poliza, Estado) 
                                    VALUES ("$idAseguradora", "$idVehiculo", "$numeroPoliza", "Activo";
        ...
        if ($conn->query($sqlInsertSeguroVehiculo) === TRUE) {
           // echo "Seguro de vehículo registrado correctamente.";
            ...
            echo '<script language="javascript">alert("Error al registrar el seguro de vehículo" );</script>. $conn->error;
        }
    } else {
        echo '<script language="javascript">alert("Error al registrar el vehículo: " );</script>' . $conn->error;
    }
